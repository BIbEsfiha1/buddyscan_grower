...
import { CheckCircleIcon, SunIcon, WaterDropIcon, BugAntIcon, AdjustmentsHorizontalIcon, RefreshIcon } from './icons/ChecklistIcons';
...
const getChecklistItemsForStage = (stage: PlantStage | undefined) => {
  switch (stage) {
    case PlantStage.SEEDLING:
      return [
        { id: 'dailyWatered', label: 'Rega', icon: <WaterDropIcon /> },
        { id: 'dailyLightAdjustment', label: 'Ajuste de Luz', icon: <SunIcon /> },
        { id: 'dailyPestCheck', label: 'Verificação de Pragas', icon: <BugAntIcon /> },
      ];
    case PlantStage.VEGETATIVE:
    case PlantStage.MOTHER: // Mother plants have similar daily needs to vegetative stage
      return [
        { id: 'dailyWatered', label: 'Rega', icon: <WaterDropIcon /> },
        { id: 'dailyNutrients', label: 'Nutrientes', icon: <AdjustmentsHorizontalIcon /> },
        { id: 'dailyLightAdjustment', label: 'Ajuste de Luz', icon: <SunIcon /> },
        { id: 'dailyPestCheck', label: 'Verificação de Pragas', icon: <BugAntIcon /> },
        { id: 'dailyRotation', label: 'Rotação', icon: <RefreshIcon /> },
      ];
    case PlantStage.FLOWERING:
      return [
        ...
        { id: 'dailyNutrients', label: 'Nutrientes', icon: <AdjustmentsHorizontalIcon /> },
        { id: 'dailyPestCheck', label: 'Verificação de Pragas', icon: <BugAntIcon /> },
        // Consider if Light Adjustment and Rotation are needed for Flowering
      ];
    case PlantStage.DRYING:
    case PlantStage.CURING:
      return []; // No standard daily checklist items during drying/curing
    default:
      return []; // Default to empty if stage is unknown or has no tasks
  }
};
