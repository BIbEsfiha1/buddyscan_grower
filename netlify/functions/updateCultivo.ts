...
export const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    ...
    console.log('[updateCultivo] Received request:', requestBody);
    ...
    console.log('[updateCultivo] Mapped update data:', updateData);
    ...
    if (Object.keys(updateData).length === 0 && requestBody.plants) {
      console.log('[updateCultivo] Only plants were sent, adding updated_at timestamp');
    ...
    console.log('[updateCultivo] Final updateData to send to Supabase:', updateData);

    // Add updated_at timestamp
    updateData.updated_at = new Date().toISOString();

    console.log(`[updateCultivo] Updating cultivo ${cultivoId} for user ${userId}`);
    ...
    console.log('[updateCultivo] Update result:', { updatedCultivo, error });
